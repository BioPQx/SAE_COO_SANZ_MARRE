...
public class UtilisateurRepository {
    ...
    public static List<Utilisateur> fromCsv(List<String[]> rows) {
        ...
        for (String[] row : rows) {
            try {
                ...
                String prenom = row[2];
                String login = row[3];
                int niveau = Integer.parseInt(row[4]);
                boolean actif = Boolean.parseBoolean(row[5]);
                LocalDate dateCreation = LocalDate.parse(row[6], formatter);

                Utilisateur user = new Utilisateur(id, nom, prenom, login, niveau, actif, dateCreation);
                users.add(user);
            } catch (Exception e) {
                e.printStackTrace(); // ignore ou log ligne invalide
            }
        }
        return users;
    }

    // Sauvegarde une liste d’utilisateurs dans le stockage
    public void saveAll(List<Utilisateur> users) throws Exception {
        for (Utilisateur u : users) {
            save(u); // ta méthode save existante
        }
    }
